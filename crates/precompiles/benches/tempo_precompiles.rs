...
fn tip20_metadata(c: &mut Criterion) {
    c.bench_function("tip20_name", |b| {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            token
                .initialize("TestToken", "T", "USD", PATH_USD_ADDRESS, admin, Address::ZERO)
                .unwrap();
    ...
    c.bench_function("tip20_total_supply", |b| {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            token.mint(admin, ITIP20::mintCall { to: user, amount: U256::from(1000) }).unwrap();
    ...
    c.bench_function("tip20_allowance", |b| {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            token
                .initialize("TestToken", "T", "USD", PATH_USD_ADDRESS, admin, Address::ZERO)
                .unwrap();
            token.approve(owner, ITIP20::approveCall { spender, amount: U256::from(500) }).unwrap();
...
fn tip20_mutate(c: &mut Criterion) {
    ...
    c.bench_function("tip20_burn", |b| {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            token
                .mint(admin, ITIP20::mintCall { to: admin, amount: U256::from(u128::MAX) })
                .unwrap();
    ...
    c.bench_function("tip20_transfer", |b| {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            token
                .mint(admin, ITIP20::mintCall { to: from, amount: U256::from(u128::MAX) })
                .unwrap();
    ...
    c.bench_function("tip20_transfer_from", |b| {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            token
                .mint(admin, ITIP20::mintCall { to: owner, amount: U256::from(u128::MAX) })
                .unwrap();
            token
                .approve(owner, ITIP20::approveCall { spender, amount: U256::from(u128::MAX) })
                .unwrap();
            ...
            b.iter(|| {
                ...
                let call =
                    black_box(ITIP20::transferFromCall { from: owner, to: recipient, amount });
    ...
    c.bench_function("tip20_set_supply_cap", |b| {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            b.iter(|| {
                let token = black_box(&mut token);
                let admin = black_box(admin);
                let call = black_box(ITIP20::setSupplyCapCall { newSupplyCap: counter });
    ...
    c.bench_function("tip20_change_transfer_policy_id", |b| {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            b.iter(|| {
                let token = black_box(&mut token);
                let admin = black_box(admin);
                let call = black_box(ITIP20::changeTransferPolicyIdCall { newPolicyId: policy_id });
...
fn tip403_registry_view(c: &mut Criterion) {
    ...
    c.bench_function("tip403_registry_policy_data", |b| {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            b.iter(|| {
                let registry = black_box(&mut registry);
                let call = black_box(ITIP403Registry::policyDataCall { policyId: policy_id });
    ...
    c.bench_function("tip403_registry_is_authorized", |b| {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            b.iter(|| {
                let registry = black_box(&mut registry);
                let call =
                    black_box(ITIP403Registry::isAuthorizedCall { policyId: policy_id, user });
    ...
    c.bench_function("tip403_registry_set_policy_admin", |b| {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            b.iter(|| {
                ...
                let admin = black_box(admin);
                let call =
                    black_box(ITIP403Registry::setPolicyAdminCall { policyId: policy_id, admin });
