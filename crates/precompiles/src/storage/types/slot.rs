...
impl<T> Slot<T> {
    ...
    /// Creates a new `Slot` from a `FieldLocation`.
    ///
    ...
    where
        T: Storable<N>,
    {
        Self::new_at_loc_with_ctx(base_slot, loc, LayoutCtx::packed(loc.offset_bytes), address)
    }

    /// Creates a new `Slot` from a `FieldLocation` generated by the `#[derive(Storable)]` macro.
    ///
    /// This is the generic way to initialize struct fields, combining layout context, slot offset
    /// and byte offset information in a type-safe manner.
    ///
    /// Useful when relying on compile-time constant evaluation to make packing/initialization choices.
    #[inline]
    pub fn new_at_loc_with_ctx<const N: usize>(
        base_slot: U256,
        loc: FieldLocation,
        ctx: LayoutCtx,
        address: Rc<Address>,
    ...
    {
        debug_assert!(
            T::IS_PACKABLE || matches!(ctx, LayoutCtx::FULL),
            "cannot pack unpackable types"
        );
        debug_assert!(
            ctx.packed_offset()
                .is_some_and(|offset| loc.offset_bytes == offset)
                || matches!(ctx, LayoutCtx::FULL),
            "`FieldLocation` and `LayoutCtx` must be consistent"
        );
        Self {
