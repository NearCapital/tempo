...
use tempo_contracts::precompiles::{FeeManagerError, PATH_USD_ADDRESS};
...
pub(crate) mod tests {
    ...
    /// Initialize PathUSD token (the first TIP20 token, required before creating other USD tokens)
    pub(crate) fn initialize_path_usd(
        storage: &mut HashMapStorageProvider,
        admin: Address,
    ) -> Result<()> {
        let mut path_usd = TIP20Token::from_address(PATH_USD_ADDRESS, storage);
        path_usd.initialize("PathUSD", "PathUSD", "USD", Address::ZERO, admin)
    }
    ...
    /// Setup factory and create a token with a separate quote token (both linking to PATH_USD)
