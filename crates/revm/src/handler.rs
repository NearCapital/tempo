    ...
    DB: alloy_evm::Database,
{
    ...
    /// presence of an aa_authorization_list in the tempo_tx_env.
    #[inline]
    ...
    fn validate_against_state_and_deduct_caller(
        ...
        let nonce_key = tx.tempo_tx_env.as_ref().map(|aa| aa.nonce_key).unwrap_or_default();
            ...
            let Some(key_auth) = &tempo_tx_env.key_authorization
        {
            ...
            if let Some(keychain_sig) = tempo_tx_env.signature.as_keychain() {
                ...
                let access_key_addr =
                    keychain_sig.key_id(&tempo_tx_env.signature_hash).map_err(|_| {
    ...
    DB: alloy_evm::Database,
{
    ...
    let aa_env =
        tx.tempo_tx_env.as_ref().expect("validate_aa_initial_tx_gas called for non-AA transaction");
