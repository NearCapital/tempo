...
pub struct TempoGenesisInfo {
    ...
    /// Timestamp of Allegro hardfork activation
    #[serde(skip_serializing_if = "Option::is_none")]
    allegro_time: Option<u64>,
...
impl TempoChainSpec {
    ...
    pub fn from_genesis(genesis: Genesis) -> Self {
        ...
        let tempo_forks = vec![
            ...
            (TempoHardfork::Allegro, allegro_time),
        ]
        .into_iter()
...
mod tests {
    ...
    fn test_parse_tempo_hardforks_from_genesis_extra_fields() {
        ...
        // Test Allegro activation
        let activation = chainspec.fork(TempoHardfork::Allegro);
        assert_eq!(
            activation,
            ForkCondition::Timestamp(4000),
            "Allegro should be activated at the parsed timestamp from extra_fields"
        );

        assert!(
            !chainspec.is_allegro_active_at_timestamp(0),
            "Allegro should not be active before its activation timestamp"
        );
        assert!(
            !chainspec.is_allegro_active_at_timestamp(1000),
            "Allegro should not be active at Adagio's activation timestamp"
        );
        assert!(
            !chainspec.is_allegro_active_at_timestamp(2000),
            "Allegro should not be active at Moderato's activation timestamp"
        );
        assert!(
            !chainspec.is_allegro_active_at_timestamp(3000),
            "Allegro should not be active at Allegretto's activation timestamp"
        );
        assert!(
            chainspec.is_allegro_active_at_timestamp(4000),
            "Allegro should be active at its activation timestamp"
        );
        assert!(
            chainspec.is_allegro_active_at_timestamp(5000),
            "Allegro should be active after its activation timestamp"
        );
    }
    ...
    fn test_tempo_hardfork_at() {
        ...
        assert_eq!(
            ...
            TempoHardfork::Allegro,
            "Should return Allegro at its activation time"
        );
        ...
        assert_eq!(
            ...
            TempoHardfork::Allegro,
            "Should return Allegro after its activation time"
        );
    }
}
