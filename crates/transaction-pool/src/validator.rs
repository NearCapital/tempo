    ...
    Client: ChainSpecProvider<ChainSpec = TempoChainSpec> + StateProviderFactory,
{
    pub fn new(
        ...
        Self { inner, aa_valid_after_max_secs, amm_liquidity_cache }
    }
    ...
    fn validate_against_keychain(
        ...
        let storage_slot =
            AccountKeychain::new().keys.at(transaction.sender()).at(key_id).base_slot();
    ...
    fn validate_one(
            ...
            Ok(valid) => {
                if !valid {
                    return TransactionValidationOutcome::Invalid(
                        transaction,
                        InvalidPoolTransactionError::other(
                            TempoPoolTransactionError::BlackListedFeePayer { fee_token, fee_payer },
                        ),
                    );
                }
            }
        ...
        match self.amm_liquidity_cache.has_enough_liquidity(fee_token, cost, &state_provider) {
            Ok(true) => {}
            Ok(false) => {
                return TransactionValidationOutcome::Invalid(
                    transaction,
        ...
        match self.inner.validate_one_with_state_provider(origin, transaction, &state_provider) {
    ...
    async fn test_blacklisted_fee_payer_rejected() {
        ...
        provider.add_account(
            ...
            ExtendedAccount::new(0, U256::ZERO).extend_storage([
                (tip20_slots::TRANSFER_POLICY_ID.into(), U256::from(policy_id)),
        ...
        let policy_set_slot = TIP403Registry::new().policy_set.at(policy_id).at(fee_payer).slot();
        ...
        let outcome =
            validator.validate_transaction(TransactionOrigin::External, transaction).await;
